...
def test_page_left_registration(selenium):
    "TC-011 Левая часть формы Регистрации содержит логотип и продуктовый слоган кабинета."
    try:
...
def test_elements_of_registr(selenium):
    """TC-012 Блок регистрации формы «Регистрация» содержит основные элементы:
...
def test_names_elements_of_registr(selenium):
    """TC-013 Названия элементов блока формы регистрации «Регистрация» соответствуют Требованию."""
    try:
        page_reg = RegistrPage(selenium)
...
def test_registr_by_valid_data(selenium):
    """TC-014 Регистрация пользователя с валидными данными"""
    page_reg = RegistrPage(selenium)
...
def test_registr_by_valid_data_(selenium):
    """TC-015 Регистрация пользователя с валидными данными: в поле ввода "Имя" и "Фамилия" - знак тире (-)."""
    page_reg = RegistrPage(selenium)
...
def test_registr_by_invalid_data(selenium):
    """TC-016 Регистрация пользователя по email, который уже был использован ранее для регистрации"""
    page_reg = RegistrPage(selenium)
    page_reg.first_name.send_keys(Settings.f_name)
    page_reg.first_name.clear()
    page_reg.last_name.send_keys(Settings.l_name)
...
def test_registr_and_redir_auth(selenium):
    """TC-017 Регистрация пользователя по email, который уже был использован ранее для регистрации
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    """TC-018 Проверка поля ввода "Имя" формы «Регистрация» валидными данными:
...
def test_first_name_by_invalid_data(selenium, invalid_first_name):
    """TC-019 Проверка поля ввода "Имя" формы «Регистрация» невалидными данными:
    пустое значение;
...
def test_last_name_by_valid_data(selenium, valid_password):
    """TC-020 Проверка поля ввода "Пароль" формы «Регистрация» валидными данными:
...
def test_passw_registration_confirm_valid_data(selenium):
    """TC-021 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» валидными данными
    (пароли совпадают)."""
    page_reg = RegistrPage(selenium)
...
def test_passw_registration_confirm_invalid_data(selenium):
    """TC-022 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» невалидными данными
    (пароли не совпадают)."""
    page_reg = RegistrPage(selenium)
    page_reg.passw_registration.send_keys(Settings.passw1)
    page_reg.passw_registration.clear()
