...
def test_page_right(selenium):
    """TC-001 В правой части формы «Авторизация» находится продуктовый слоган ЛК "Ростелеком ID"."""
    try:
        page = AuthPage(selenium)
        assert 'Персональный помощник в цифровом мире Ростелекома' in page.page_right.text
    except AssertionError:
        print('Отсутствует элемент в правой части формы')


def test_elements_of_auth(selenium):
    """TC-002 Блок аутентификации формы «Авторизация» содержит элементы (Меню выбора типа аутентификации,
...
def test_menu_of_type_auth(selenium):
    """TC-003 Меню выбора типа аутентификации содержит: 'Номер', 'Почта', 'Логин', 'Лицевой счёт'."""
    try:
        page = AuthPage(selenium)
...
def test_menu_of_type_active_auth(selenium):
    """TC-004 В Меню выбора типа аутентификации по умолчанию выбрана форма аутентификации по телефону."""
    page = AuthPage(selenium)
...
def test_placeholder_name_of_user(selenium):
    """TC-005 В форме ввода ('Номер', 'Почта', 'Логин', 'Лицевой счёт') подсказка внутри поля формы меняется
...
def test_forgot_password_link(selenium):
    """TC-006 Проверка перехода по ссылке 'Забыл пароль'."""
    page = AuthPage(selenium)
...
def test_register_link(selenium):
    """TC-007 Проверка перехода по ссылке 'Зарегистрироваться'."""
    page = AuthPage(selenium)
...
def test_auth_by_valid_email_pass(selenium):
    """TC-008 Аутентификация пользователя с валидным email и паролем."""
    page = AuthPage(selenium)
...
def test_auth_by_invalid_email(selenium, incor_email, incor_passw):
    """TC-009, TC-10 Аутентификация пользователя с невалидным email и паролем:
